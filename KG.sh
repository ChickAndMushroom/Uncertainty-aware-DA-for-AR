...
#====== parameters ======#
dataset=kinetics_gameplay # hmdb_ucf | hmdb_ucf_small | ucf_olympic | kinetics_gameplay
...
#====== select dataset ======#
path_data_root=/media/pjm/2ce2df73-4999-456d-9a8f-12f8f3a7686a/dataset/ # depend on users
path_exp_root=G2KwithLSTM/ # depend on users

if [ "$dataset" == "hmdb_ucf" ] || [ "$dataset" == "hmdb_ucf_small" ] ||[ "$dataset" == "ucf_olympic" ]||[ "$dataset" == "kinetics_gameplay" ]
then
	dataset_source=gameplay_train # depend on users
	dataset_target=kinetics_train # depend on users
	dataset_val=kinetics # depend on users
	num_source=2625 # number of training data (source) too many for K;43378
...
then
	...
	#------ main command ------#
	python mainCADA.py $class_file $modality $train_source_list $train_target_list $val_list --exp_path $exp_path \
...
then
	...
	python test_models.py $class_file $modality $train_source_list $train_target_list\
	...
	-j 4 --bS $bS --bS_2 $bS_2 --top 1 3 5 --add_fc 1 --fc_dim $fc_dim --baseline_type $baseline_type --frame_aggregation $frame_aggregation 
