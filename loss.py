...
class BayesLoss(nn.Module):
    ...
    def __init__(self, num_sims, dim):
        super(BayesLoss, self).__init__()
        self.num_sims = num_sims
        self.dim = dim
        

    def forward(self, pred_logit, true_label, var, const):
        
        std = torch.sqrt(var)
        num = pred_logit.size()[0]
        #
        # variance depressor = exp(variance) - 1
        variance_depressor = torch.mean(torch.sub(torch.exp(var),const)) 
        # undistorted loss
        criterion = nn.CrossEntropyLoss().cuda()
        undistorted_loss = criterion(pred_logit, true_label) 
        # Monte Carlo simulations
        mean0 = torch.zeros([num, self.dim]).cuda() #tensor for average = 0
        stde = std.expand(num, self.dim).cuda()
        elu = nn.ELU()
        # distorted loss = Cross_entropy(pred_logit + sampled noise, true label)
        distorted_loss = criterion(torch.add(pred_logit, torch.normal(mean=mean0, std=stde).cuda()), true_label)
        # diff = Elu(undistorted loss - distorted loss)
        monte_carlo_results = elu(undistorted_loss - distorted_loss)
        #print('t1',monte_carlo_results)
        for i in range(self.num_sims - 1):
            # distorted loss = Cross_entropy(pred_logit + sampled noise, true label)
            distorted_loss = criterion(torch.add(pred_logit, torch.normal(mean=mean0, std=stde).cuda()), true_label)
            # diff = Elu(undistorted loss - distorted loss)
            monte_carlo_results = monte_carlo_results.clone() + elu(undistorted_loss - distorted_loss)
        # variance loss = undistorted loss * mean(monte_carlo_results)
        variance_loss = torch.mul(undistorted_loss, torch.div(monte_carlo_results, self.num_sims))
